const NotifyBox = ({ title, date, desc, seen,id,MarkAsRead, DeleteNote,read }) => {
  ...
  return (
    <div className="container mt-5 mb-2 p-3 notifybox rounded position-relative">
      <div className="d-flex-between mb-3 bg-light rounded ">
        ...
        { (seen) && <span className="mr-3 align-items-center d-none d-md-flex">
       ...
       {   (read) &&  ((read == "false") ? <span className="ml-1 py-1 px-2 rounded" style={{backgroundColor:"#EEE82C"}}>unread</span>:<span className=" px-2 py-1 ml-1 rounded" style={{backgroundColor:"#91CB3E"}}>read</span>)
        }</div>
      </div>
      <div className="fontsize14">{desc}</div>
 {  (read) && ( (read==="false") && <div className="mark-as-read py-1 px-2 rounded bg-danger text-light" onClick={()=>{MarkAsRead(info_id)}}>mark as read</div>)} 
   {  read &&( (read === "true") &&   <div className="delete" onClick={()=>{DeleteNote(info_id)}}><span class="material-symbols-outlined">
delete
</span></div>)}
    </div>
  ); 
